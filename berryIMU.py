...
while True:
    ...
    if 1:                       #Change to '0' to stop showing the angles from the accelerometer
        if (AccXangle < 0 and x_pos) or (AccXangle > 0 and x_pos == False):
            x_pos = not x_pos
            outputString += 'Forward Push Detected'
            time.sleep(1)
        elif (AccYangle < 0 and y_pos) or (AccYangle > 0 and y_pos == False):
            y_pos = not y_pos
            outputString += 'Upward Lift Detected'
            time.sleep(1)
        else:
            outputString += 'None Detected'#"#  ACCX Angle %5.2f ACCY Angle %5.2f  #  " % (AccXangle, AccYangle)
